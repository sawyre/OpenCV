...
public:
	...
	IntegMas(Mat &rsh) {
		...
		for (int i = 0; i < 3; i++) {
			arr[i] = new int *[rsh.rows];
			arr[i][0] = new int [(rsh.cols * rsh.rows)];
			for (int j = 1; j < rsh.rows; j++) {
				arr[i][j] = arr[i][j - 1] + rsh.cols - 1;
			}
		}
		int **tmpB = arr[0];
		int **tmpG = arr[1];
		...
		for (it = rsh.begin<Vec3b>(), end = rsh.end<Vec3b>(); it != end; ++it) {
			**tmpB = (*it)[0];
			**tmpG = (*it)[1];
			**tmpR = (*it)[2];
			tmpB++;
	...
	int getXY(int color, int x, int y) { return arr[color][x][y]; }
private:
	int **arr[3];
};
